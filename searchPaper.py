...
headers = {
    ...
    "Referer": "https://dblp.org/search?q=NDSS",
    "Connection": "close",
...
def get_all_bibtex(url):
    ...
    bibtex_urls = re.compile(
        r"https://dblp\.org/rec/conf/[a-zA-Z0-9]{1,10}/[a-zA-Z0-9]{1,30}\.html\?view=bibtex"
    ).findall(data)
...
def main():
    ...
    for year in range(year_start, year_end):
        for key in lib:
            ...
            url = "https://dblp.org/db/conf/{name}/{name}{year}.html".format(
                name=name, year=year
            )
